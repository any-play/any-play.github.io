;(() => {
  ...
  function parseCIDR(CIDR) {
  	...
  	let beg = CIDR.substr(CIDR,CIDR.indexOf('/'))
  	let off = (1 << (32 - parseInt(CIDR.substr(CIDR.indexOf('/') + 1)))) - 1
  	let sub = beg.split('.').map(a => ~~a)

  	//An IPv4 address is just an UInt32...
  	let buf = new ArrayBuffer(4) //4 octets
  	...
  	let end = Array(...new Uint8Array(buf)).reverse()
